...
from app.models import Pet, Breed, AdoptionApplication, ApplicationStatusEnum, PetImage, Shelter, Species
...
class PetService:
    ...
    def get_pet_details(pet_id):
        selected_pet = db.session.query(Pet).filter(Pet.pet_id == pet_id).first()
        ...
        selected_pet_image_urls_tuples = db.session.query(PetImage.image_url, PetImage.sort_order).filter(pet_id == PetImage.pet_id).order_by(PetImage.sort_order).all()

        selected_pet_image_urls = [{'image_url': image_url, 
                                    'sort_order': sort_order} 
                                    for image_url, sort_order in selected_pet_image_urls_tuples]

        selected_pet_dict = {'petId': str(uuid.UUID(bytes=pet_id)), 
        ...
        return selected_pet_dict

    @staticmethod
    def register_pet(pet_data, pet_images):
        files = []
    ...
    @staticmethod
    def edit_pet(pet_data, pet_images):
